...
# ===== NEW: ALLC Processing =====
# Convert ALLC files to COV and separate contexts (CG, CH)
global_stats = ms2.pp.batch_process_allc(
    allc_dir='raw/allc',
    output_base_dir='processed',
    contexts=['CG', 'CH', 'all'],
    n_jobs=100  # Parallel processing
)
...
- **ALLC Processing**: Convert ALLC to COV with context separation (CG, CH)
- **Global Methylation**: Calculate global methylation levels for each context
...
- **Parallel Processing**: Multi-threaded support (100+ jobs)
